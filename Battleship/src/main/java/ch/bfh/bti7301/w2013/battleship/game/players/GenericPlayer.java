...
public class GenericPlayer implements Player {
	...
	/**
	 * Called after we received a missile from an opponent (on player)
	 */
	public Missile placeMissile(Missile m) {
		// Do some sanity checks
		if (!this.getBoard().withinBoard(m.getCoordinates())) {
			throw new RuntimeException(
					"Coordinates of missile not within board:" + m.getCoordinates());
		}
		...
		for (Ship s : this.getBoard().getPlacedShips()) {
			for (Coordinates c : s.getCoordinatesForShip()) {
				if (c.equals(m.getCoordinates())) {
					...
					// Now everything is evaluated, place the missile on our own board
